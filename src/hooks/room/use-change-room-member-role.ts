...
import { changeRoomMemberRole } from "@/lib/api/rooms"
import type { ChangeRoomMemberRoleInput } from "@/types/room.types"

export function useChangeRoomMemberRole() {
  const { userId } = useRoomSession()
  const api = useRoomApiClient()
  const invalidateRooms = useRoomInvalidation()

  return useMutation({
    mutationFn: (input: ChangeRoomMemberRoleInput) => {
      if (!userId) throw new RoomSessionRequiredError()
      return changeRoomMemberRole(api, input)
    },
